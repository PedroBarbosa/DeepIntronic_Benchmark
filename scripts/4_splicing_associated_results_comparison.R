...
df <- map2_df(list_files, 
              ...
              ~read_tsv(.x) %>% 
                ...
                select(c('tool', 'norm_mcc', 'weighted_norm_mcc', 'weighted_F1', 'total_p', 'total_n',
                         'tp', 'fp', 'tn', 'fn',
                         'fraction_nan', 'weighted_accuracy', 'auROC', 'auPRC', 'analysis'))) 

# Remove AP score from tools with more than 50% of missing data since ROC curves are not drawn in such cases
df$auPRC <- ifelse(df$tool == "MLCsplice" & df$analysis == "branchpoint", NA, df$auPRC)
df$auPRC <- ifelse(df$tool == "IntSplice2" & df$analysis == "branchpoint", NA, df$auPRC)
...
###### Per region #########
###########################
ggplot(df %>% filter(analysis != "branchpoint"), aes(x=group, y=auPRC,fill=group)) +
...
ggplot(df_, aes(x=reorder(tool, -auPRC, median))) +
  geom_boxplot(aes(x=group, y=auPRC, fill=group, alpha=0.5), outlier.shape=NA) + 
  geom_point(aes(x=group, y=auPRC, fill = region), position = position_dodge(width=0.2), size=3, shape = 21)  +
  ...
  facet_wrap(reorder(tool, -auPRC, median) ~ ., nrow=2) +
  theme_bw() +
  ylim(0.2, 1.21) +
  ylab('auPRC') +
  theme(legend.title=element_blank(),
...
pe_all$tool = factor(pe_all$tool, levels = df %>% filter(analysis == "pe_acceptor_associated") %>% arrange(-auPRC) %>% pull(tool))
...
all$tool <- factor(all$tool, levels = all %>% filter(analysis == "pe_acceptor_associated") %>% arrange(-auPRC) %>% pull(tool))
...
ggplot(all, aes(x=label, y=auPRC, group=tool, color =tool)) +
...
fill_missing_analysis <- function(i){
  ...
  df_ <- list_dfs[[i]] %>% select(tool,auROC, auPRC)
  ...
  not_perfomed <- tibble(tool = missing_tools, 
                           ...
                           auPRC = rep(c(0.18), times = length(missing_tools)))
...
# out <-out %>% mutate(auPRC=case_when(tool=="MLCsplice" & analysis == "pe_new_donor" ~ 0.19,
#                                    tool=="dbscSNV" & analysis %in% c("pe_new_donor", "partial_ir_new_donor") ~ 0.19,
#                                    TRUE ~ auPRC))
out <-out %>% mutate(auPRC=case_when(tool=="IntSplice2" & analysis %in% c("partial_ir_acceptor_associated", "pe_acceptor_associated") ~ 0.19,
                                                 TRUE ~ auPRC))

out <-
  out %>% mutate(auPRC_bins = cut(
    auPRC,
...
final_heat <- out %>% select(-auROC, -auPRC) %>% pivot_wider(names_from = analysis, values_from = auPRC_bins) %>% column_to_rownames(var = 'tool')
final_heat <- out %>% select(-auROC, -auPRC_bins) %>% pivot_wider(names_from = analysis, values_from = auPRC) %>% column_to_rownames(var = 'tool')
