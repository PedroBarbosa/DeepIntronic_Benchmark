...
data_pathogenic$offset[which(data_pathogenic$HGVSc == "ENST00000396625.3:c.72-26_72-23del")] <- 26
data_pathogenic$offset[which(data_pathogenic$HGVSc == "ENST00000278616.4:c.2839-579_2839-576del")] <- 579
...
data_benign$offset[which(data_benign$HGVSc == "ENST00000280362.3:c.84-715_84-713del")] <- 715
...
data_benign$offset[which(data_benign$HGVSc == "ENST00000375708.3:c.98-1019_98-1018insAT")] <- 1019
...
data_benign$offset[which(data_benign$HGVSc == "ENST00000298542.4:c.206-60_206-59del")] <- 60
...
#write.table(file = "pos_patho_pseudoexon.txt", x = pseudoexon$POS, quote=F, row.names = F, col.names = F)

partial_ir <- data_pathogenic %>% filter((!str_detect(Functional_consequence, 'pseudoexon')) & (str_detect(Functional_consequence, 'exon_extension') | 
                                                                                           str_detect(Functional_consequence, 'intron_retention')))
partial_ir$group <- "Partial intron retention"
#write.table(file = "pos_patho_partial_ir.txt", x = partial_ir$POS, quote=F, row.names = F, col.names = F)

df_pathogenic <- bind_rows(pseudoexon, partial_ir) %>% select(c(Effect_category, Functional_consequence, offset, group))


# Selet benign variants according to the offset
benign_to_partial_ir <- head(data_benign, nrow(partial_ir))
benign_to_partial_ir$group <- "Partial intron retention"
#write.table(file = "pos_benign_partial_ir.txt", x = benign_to_partial_ir$POS, quote=F, row.names = F, col.names = F)

benign_to_pseudoexon <- tail(data_benign, nrow(data_benign) - nrow(benign_to_partial_ir)) %>% filter(offset < 10000) %>% sample_n(size=nrow(pseudoexon))
...
df_pathogenic %>% drop_na(offset) %>% 
  ggplot() + 
  geom_density(aes(x=log10(offset), y=after_stat(scaled), fill=group), alpha = 0.8, position = "identity") +
...
#######################################
setwd("~/git_repos/giga_science_reviews//data/splicing_pathogenic_manual_curation/")
...
#pal <- c("#A6CEE3", "#F1E2CC", "#A6761D", "#FC8D62", "#7FC97F", "#0430B5", "#984EA3", "#999999", , "darkgreen")
pal <- c("#A6CEE3", "#F1E2CC", "#A6761D", "#FC8D62", "#FFED6F", "#7FC97F", "#0430B5", "#984EA3")
...
df_noPDIVAS_andAbSplice <- df %>% filter(tool != "PDIVAS" & tool != "AbSplice-DNA")
...
ggplot(df_noPDIVAS_andAbSplice, aes(x = analysis, y=weighted_norm_mcc)) + 
